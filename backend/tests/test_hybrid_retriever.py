...
class TestHybridRetriever(unittest.TestCase):
    def setUp(self):
        ...
        self.dim = 8
        self.chunks = [
            ChunkModel(id=f"c{i}", content=f"chunk number {i}", rich_content="", metadata={}) for i in range(100)
        ]
        # Random embeddings
    ...
    def test_search_correctness(self):
        ...
        # Run search
        results = self.retriever.search(query="chunk", query_embedding=query_emb, top_k=5)
    ...
    def test_search_missing_vector_candidates(self):
        ...
        results = self.retriever.search(query=query_text, query_embedding=query_emb, top_k=20)
